...
```{r read df.raw}
df.raw <-  read.csv('../../4_Data_Extraction/4_3_Formal_Data/Formal_3.csv',
                    header = T, sep=",", stringsAsFactors = FALSE) %>%
  dplyr::mutate(Subject = index) %>%
  dplyr::select(
    Subject, Gender, Age, Edu, Marriage, 
    Blue1, Blue2, Blue3, Blue4, Blue5, Blue6, Blue7,
    everything()
  ) %>%
  dplyr::select(-totalseconds, -index, -totalvalue, -T1, -T2, -T3, -T4, -T5, -T6)
```
...
df.mean <- df.raw %>%
  dplyr::mutate(
    Blue_mean = rowMeans(select(., starts_with("Blue"))),
    ASC_mean = rowMeans(select(., starts_with("ASC"))),
...
alpha <- psych::alpha(df.raw[, c("OSC1", "OSC2", "OSC3", "OSC4", "OSC5", "OSC7")])
alpha$total$raw_alpha
...
alpha <- psych::alpha(df.raw[, c("UI1", "UI2", "UI3")])
alpha$total$raw_alpha

alpha <- psych::alpha(df.raw[, c("UC1", "UC2", "UC3")])
alpha$total$raw_alpha

alpha <- psych::alpha(df.raw[, c("DI1", "DI2", "DI3")])
alpha$total$raw_alpha

alpha <- psych::alpha(df.raw[, c("DC1", "DC2", "DC3")])
alpha$total$raw_alpha

alpha <- psych::alpha(df.raw[, c("Blue1", "Blue2", "Blue3", "Blue4", "Blue5", "Blue6", "Blue7")])
alpha$total$raw_alpha
```
