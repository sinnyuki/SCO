...
```{r read df.raw}
df.raw <-  read.csv('../../4_Data_Extraction/4_2_Formal_Data/4_2_1_Demo/Demo.csv',
                    header = T, sep=",", stringsAsFactors = FALSE) %>%
  dplyr::mutate(Subject =index) %>%
  dplyr::select(Subject, Gender, Age, Edu, Marrige, everything()) %>%
...
df.mean <- df.raw %>%
  ...
  dplyr::select(Subject, Gender, Age, 
...
result.CFA <- capture.output({
  CFA <- 'ASC =~ ASC1 + ASC2 + ASC3 + ASC4
          OSC =~ OSC1 + OSC2 + OSC3 + OSC4 + OSC5
          SCO =~ ASC + OSC
